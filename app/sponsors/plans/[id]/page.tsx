...
export default function SponsorshipCheckoutPage() {
  ...
  const totalPrice = basePackage?.amount || 0;
  ...
  const handleRazorpayPayment = async (registrationId: string) => {
    try {
      ...
      const options = {
        ...
        name: "Wiswora Event",
        description: `Sponsorship for ${basePackage.name}`,
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      alert('Sponsorship request failed. Please try again.');
      setSubmitting(false);
    }
  };
  ...
  const createPayPalOrder = async () => {
    if (!formData.name || !formData.email || !formData.organization) {
      alert('Please fill out the organization details first.');
  ...
  if (success) {
    return (
      <main className="min-h-screen bg-midnight flex flex-col items-center justify-center text-center px-4">
        ...
        <h1 className="text-4xl font-bold text-white mb-4">Sponsorship Request Received!</h1>
        <p className="text-muted-foreground max-w-md mb-8">
          Thank you for choosing to partner with us. Our team will review your details and contact you shortly with the next steps.
        </p>
        ...
        <div className="pt-20 pb-16 px-6">
          <div className="container mx-auto max-w-6xl">
            <h1 className="text-3xl md:text-5xl font-heading font-bold text-white mb-10 text-center">
              Confirm your <span className="text-primary">Sponsorship</span>
            </h1>
                  ...
                  <form id="checkout-form" onSubmit={handleFormSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="col-span-2">
                        <label className="block text-sm font-medium text-foreground mb-1">Company / Organization Name</label>
                        <input required type="text" value={formData.organization} onChange={e => setFormData({...formData, organization: e.target.value})} className="w-full bg-background border border-border rounded-lg px-4 py-3 text-foreground focus:ring-2 focus:ring-primary/50 outline-none" placeholder="Acme Corp" />
                      </div>
                      <div className="col-span-2 md:col-span-1">
                        <label className="block text-sm font-medium text-foreground mb-1">Contact Person Name</label>
                      ...
                      <div className="col-span-2 md:col-span-1">
                        <label className="block text-sm font-medium text-foreground mb-1">Contact Email Address</label>
                        <input required type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-background border border-border rounded-lg px-4 py-3 text-foreground focus:ring-2 focus:ring-primary/50 outline-none" placeholder="john@acmecorp.com" />
                      </div>
                    </div>
                  </form>
                </div>
                ...
                <div className="bg-gradient-to-b from-primary/10 to-transparent border border-primary/20 p-8 rounded-3xl sticky top-24">
                  ...
                  <div className="space-y-4 mb-8">
                    <div className="flex justify-between items-start pb-4 border-b border-white/10">
                      ...
                      <span className="font-medium text-white">{displayCurrency}{totalPrice}</span>
                    </div>
                  </div>

                  <div className="flex justify-between items-center mb-8">
                    <span className="text-lg text-foreground">Total Commitment</span>
                  ...
                  {paymentMethod === 'Razorpay' ? (
                    ...
                    >
                      {submitting ? <Loader2 className="w-5 h-5 animate-spin" /> : `Commit ${displayCurrency}${totalPrice}`}
                    </button>
                  ) : (
                    <div className="mt-4">
                      <PayPalButtons 

...
export default function Page() {
  ...
  const [formData, setFormData] = useState<any>({ category: 'Chairs' });
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const res = await fetchApi('/organizers?category=Chairs');
      setData(res);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = { ...formData, category: 'Chairs' };
      if (editingId) {
  ...
  const handleEdit = (item: any) => {
    setFormData({ ...item, category: 'Chairs' });
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      <PageHeader 
        title="Chairs" 
        description="Manage chairs"
        breadcrumbs={[{ label: 'Admin', href: '/admin' }, { label: 'Chairs', href: '/admin/chairs' }]}
        action={
          <button onClick={() => { setFormData({ category: 'Chairs' }); setEditingId(null); setShowModal(true); }} className="flex items-center px-4 py-2 rounded-md bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium transition-colors shadow-lg">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-md shadow-2xl">
            <h3 className="text-xl font-semibold text-foreground mb-4">{editingId ? 'Edit' : 'Add'} Chair</h3>

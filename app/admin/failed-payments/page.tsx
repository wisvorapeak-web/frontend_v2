...
export default function Page() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const res = await fetchApi('/failedpayments');
      setData(res);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await fetchApi(`/failedpayments/${editingId}`, {
          method: 'PUT',
          body: JSON.stringify(formData)
        });
      } else {
        await fetchApi('/failedpayments', {
          method: 'POST',
          body: JSON.stringify(formData)
        });
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetchApi(`/failedpayments/${id}`, { method: 'DELETE' });
      loadData();
    } catch (error) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      <PageHeader 
        ...
        description="Manage failed payments"
        breadcrumbs={[{ label: 'Admin', href: '/admin' }, { label: 'Failed Payments', href: '/admin/failed-payments' }]}
        action={
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-md shadow-2xl">
            <h3 className="text-xl font-semibold text-foreground mb-4">{editingId ? 'Edit' : 'Add'} Failed Payments</h3>
